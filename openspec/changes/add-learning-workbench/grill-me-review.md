...
5. **重跑时已写入笔记会不会重复？**
   - 结论：不会。pending 保持顺序，笔记使用日期+内容指纹的稳定 operation ID；同一内容重试复用结果，不同内容不会与当天旧序号冲突。假 MCP 以同一幂等语义重跑后最终只有两篇笔记和一个索引。
...
1. **同一天重新筛选出不同内容时，日期+序号 operation ID 是否仍然安全？**
   - 生产首轮验证发现：不安全。当天旧批次已占用第一个序号，新批次使用相同 operation ID、不同参数时被 Inkstone 以 409 拒绝。
   - 修复：笔记 operation ID 改为日期+内容键指纹；同一内容保持稳定，不同内容得到不同 ID，并保留无内容键旧 pending 的序号兼容路径。
   - 证据：`uses different stable operation ids for different content on the same date` 与生产失败记录先暴露问题，提交 `45a302a` 修复。

2. **笔记修复后，索引刷新是否存在相同冲突？**
   - 生产第二次续跑发现：存在。8 篇笔记已幂等完成，但索引更新的固定 `v1` operation ID 已被不同参数占用。
   - 修复：索引创建、更新和整理 operation ID 均绑定索引正文指纹；精确重试仍复用同一结果，内容变化获得新 ID。
   - 证据：索引指纹断言与生产断点续跑，提交 `4737f66` 修复。

3. **失败是否导致半成品索引或丢失已有资料？**
   - 结论：没有。第一次停在首篇写入前，第二次停在索引更新前；已有索引保持原版本，失败上下文写入运行记录。第三次直接复用 pending、8 篇报告缓存和已创建笔记，成功更新索引后才写入成功日期并清理 pending/cache。

4. **最终线上内容是否满足输出契约？**
   - 结论：满足。MCP API 读取当日索引确认其链接 8 篇独立笔记；8 篇均具有至少 3 个中文标签、原始 HTTPS 来源链接和“为什么值得学习”“哪些方面做得好”“动手练习”等章节。

5. **生产定时器是否真正收敛且可回滚？**
   - 结论：是。`inkstone-tech-digest.timer` 已启用并排定次日执行，两个旧 timer 均为 disabled/inactive；切换状态保存在独立目录，部署前 unit、状态与 Git 信息另有时间戳备份。

### 生产验证结果

- 部署提交：`4737f66`。
- VPS 定向测试：17/17 通过。
- 运行结果：`succeeded`，`selected=8`、`writtenNotes=8`、`writtenIndex=1`。
- 内容核验：索引链接 8/8，中文标签 8/8，原始链接 8/8，学习章节 8/8。
- 运行提示：GitHub Trending 镜像一次拉取失败，但其余来源仍形成 172 条候选、去重后 56 条，属于非致命来源告警。
- 数据安全：未删除历史笔记、凭据或旧状态；部署前备份位于 `/root/backups/inkstone-learning-library-20260922164358`。

### 结论

生产部署复审通过。两处只在“同日不同内容重跑”才会触发的 operation ID 冲突均由真实生产证据暴露、补测试并修复；最终断点续跑成功，定时器和内容输出符合 OpenSpec 验收条件。
